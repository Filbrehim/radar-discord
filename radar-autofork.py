...
async def afficher(res,demande,message) :
   if res['score'] :
      ...
      for champ in ['type','compact','vendeur'] :
          if champ in res.keys() :
              await message.channel.send("{0} : **{1}**".format(champ,res[champ])) 
      if 'detail' in res.keys() :
          for d in res['detail'] :
              await message.channel.send("{0} : **{1}**".format(d,res['detail'][d]))
...
async def webhook(res,demande,message) :
   ...
   if os.path.isdir(channel_dir) :
      ...
      if os.path.exists(channel_dir+"/webhook.txt") :
         ...
         await afficher(res,demande,message)
   else :
